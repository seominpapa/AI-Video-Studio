...
2. 첨부자료의 브랜드, 톤, 색상, 스타일 단서를 확인하고 루트 `DESIGN.md`와 비교합니다. 충돌하면 사용자 확인 또는 작업별 디자인 브리프로 우선순위를 기록합니다.
3. 초기 영상 생성 요청에는 음성 파일과 스크립트 파일을 함께 첨부하도록 안내합니다. 둘 중 하나라도 없으면 제작 전에 사용자에게 추가 요청합니다.
4. `YYYYMMDD_작업제목` 폴더를 만들고 하위 폴더를 구성합니다. 스크립트 파일용 `script/`와 서브에이전트 지시가 필요할 때의 `agent-briefs/`도 함께 만듭니다.
5. 음성 원본 파일과 필요 시 변환한 기준 음성 파일을 작업 폴더의 `audio/` 안에 둡니다. 사용자가 첨부 파일명을 미리 맞출 필요는 없습니다.
6. 첨부 스크립트 파일을 `script/` 안으로 옮기고, `script/narration-script.txt` 정리본을 만듭니다.
7. 정리본은 마크다운, 코드 블록, 촬영 메모, `[화면 예시]` 같은 화면 전용 요소를 제거하고, 영어와 숫자는 한국어 발음/표기로 바꾸며, 대시는 삭제하고 화살표는 쉼표로 바꿉니다.
8. 정리본은 한 줄이 한 호흡이 되도록 약 20자 초과 줄을 의미 단위와 쉼표 위치에서 나누고, 모든 줄 뒤에 빈 줄을 둡니다.
9. 음성 길이를 확인합니다.
10. 스크립트를 장면 단위 타임라인으로 나눕니다.
11. 장면마다 핵심 메시지, 인포그래픽 구조, 텍스트 라벨을 정합니다.
12. Remotion 또는 HyperFrames로 구현합니다.
13. 대표 프레임을 `review-frames/`에 렌더링해 텍스트 겹침과 타이밍을 확인합니다.
14. 싱크 불일치가 있으면 전체 타임라인을 1초 단위와 장면 전환 전후 프레임으로 샘플링하고 컨택트 시트를 만들어 확인합니다.
15. 최종 MP4를 `outputs/`에 렌더링합니다.
16. 비디오와 오디오 트랙 포함 여부를 확인합니다.
