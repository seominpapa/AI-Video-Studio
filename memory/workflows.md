...
2. `https://github.com/VoltAgent/awesome-design-md`의 컬렉션 또는 `design-md/` 폴더를 확인해 선택 가능한 `DESIGN.md` 종류를 사용자에게 리스트업합니다. 사용자가 선택하지 않거나 기본값을 요청하면 루트 `DESIGN.md`만 사용합니다.
3. 외부 `DESIGN.md`가 선택되면 선택한 이름, 출처 URL, 적용할 색상/타이포그래피/레이아웃 단서를 작업별 디자인 브리프에 기록합니다.
4. 첨부자료의 브랜드, 톤, 색상, 스타일 단서를 확인하고 루트 `DESIGN.md` 및 선택된 외부 `DESIGN.md`와 비교합니다. 충돌하면 사용자 확인 또는 작업별 디자인 브리프로 우선순위를 기록합니다.
5. 초기 영상 생성 요청에는 음성 파일과 스크립트 파일을 함께 첨부하도록 안내합니다. 둘 중 하나라도 없으면 제작 전에 사용자에게 추가 요청합니다.
6. `YYYYMMDD_작업제목` 폴더를 만들고 하위 폴더를 구성합니다. 스크립트 파일용 `script/`와 서브에이전트 지시가 필요할 때의 `agent-briefs/`도 함께 만듭니다.
7. 음성 원본 파일과 필요 시 변환한 기준 음성 파일을 작업 폴더의 `audio/` 안에 둡니다. 사용자가 첨부 파일명을 미리 맞출 필요는 없습니다.
8. 첨부 스크립트 파일을 `script/` 안으로 옮기고, `script/narration-script.txt` 정리본을 만듭니다.
9. 정리본은 마크다운, 코드 블록, 촬영 메모, `[화면 예시]` 같은 화면 전용 요소를 제거하고, 영어와 숫자는 한국어 발음/표기로 바꾸며, 대시는 삭제하고 화살표는 쉼표로 바꿉니다.
