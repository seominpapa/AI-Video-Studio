...
사용자가 Remotion 또는 HyperFrames를 직접 지정하지 않아도 됩니다. Codex는 첨부한 스크립트와 영상 목적을 보고 더 적합한 도구를 추천할 수 있습니다.
...
6. 음성 파일 길이 확인
7. 필요하면 로컬 전사 실행
8. 장면별 타임라인 작성 및 `timeline/scenes.json` 기준 파일 저장
9. 스크립트와 목적에 맞는 도구 추천 또는 사용자 지정 도구 확정
10. Remotion 또는 HyperFrames 코드 작성
11. 대표 프레임 렌더링 및 레이아웃/싱크 검수
12. 최종 MP4 렌더링 후 길이, 오디오 트랙, 디버그 UI 잔존 여부 확인
...
- 음성 파일과 원본 스크립트 파일을 함께 첨부하는 것을 기본으로 합니다. 둘 중 하나가 빠지면 최종 제작 전에 먼저 추가 요청합니다.
