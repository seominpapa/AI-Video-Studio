...
Voicebox 샘플 업로드는 WAV, MP3, FLAC, WebM 형식을 안내하고 있습니다. 이 저장소에서 사용할 최종 내레이션은 가능하면 WAV로 저장하되, 저장 파일명은 자유롭게 두어도 됩니다. 사용자 음성 샘플을 기반으로 음성 파일을 자동 생성할 수는 있지만, 직접 녹음한 원본 음성보다 발음, 감정, 호흡, 억양 품질이 떨어질 수 있습니다. 중요한 영상은 생성 음성을 한 번 들어보고 필요하면 재생성하거나 직접 녹음본으로 교체합니다. 구간별 볼륨이 갑자기 튀는 경우가 있으므로 Voicebox로 만든 음성은 영상 싱크 작업 전에 FFmpeg loudness normalization을 적용하는 것을 권장합니다.
...
7. 볼륨이 튀는 것을 줄이기 위해 FFmpeg로 내레이션 기준 `-16 LUFS` 정규화 파일을 만듭니다.
8. 생성된 원본 음성과 정규화된 음성 파일을 영상 제작 요청 메시지에 첨부합니다.

`voice.wav`를 후처리하는 예시는 아래와 같습니다.

```powershell
ffmpeg -i voice.wav -af loudnorm=I=-16:TP=-1.5:LRA=11 voice_normalized.wav
```

유튜브 내레이션은 보통 `-16 LUFS` 정도가 안정적입니다. 배경음악까지 합친 최종 영상은 필요하면 전체 믹스를 `-14 LUFS` 근처로 맞추는 별도 단계를 둡니다.

Voicebox로 만든 음성을 사용할 때도 사용자가 파일명을 맞출 필요는 없습니다. Codex는 첨부한 음성 파일을 작업 폴더의 `audio/` 안에 정리하고, 정규화된 파일이 있으면 그 파일을 기준으로 길이 확인, 로컬 전사, 장면 타임라인, 싱크 검증을 진행합니다.
...
- 기준 음성 파일은 작업 폴더의 `audio/` 안에서 관리합니다. 사용자가 파일명을 `voiceover.wav`로 맞출 필요는 없습니다. Voicebox 등 TTS 음성은 필요하면 `ffmpeg -i voice.wav -af loudnorm=I=-16:TP=-1.5:LRA=11 voice_normalized.wav`로 정규화한 파일을 기준 음성으로 사용합니다.
- 장면 타임라인이 확정되면 `timeline/scenes.json`에 저장하고, Remotion, HyperFrames, QA가 같은 장면 수, 순서, 시작/종료 시간을 기준으로 작업합니다.
- 모션그래픽은 인포그래픽 중심 레퍼런스처럼 인포그래픽과 짧은 텍스트가 함께 의미를 만들도록 구성하며, 전체 내레이션을 자막처럼 계속 노출하지 않습니다.
- 화면 하단 18-22%는 자막 안전영역으로 먼저 비워두고, 핵심 인포그래픽과 라벨은 그 위에서 완결되게 배치합니다.
...
- 텍스트와 인포그래픽은 같은 시각 영역에 겹쳐 얹지 않습니다. 기본 구조는 상단 설명 영역, 중앙 인포그래픽, 하단 자막 안전영역처럼 역할별 안전 영역을 분리합니다.
- 하단 자막 안전영역에는 핵심 인포그래픽, 라벨, 장식, 진행 막대를 배치하지 않습니다. 라벨은 인포그래픽 내부, 상단 설명 아래, 또는 측면 트레이에 둡니다.
...
- 하단 자막 안전영역은 먼저 비워두고, 인포그래픽과 라벨은 그 위에서 완결합니다.
