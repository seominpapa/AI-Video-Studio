...
영상 제작을 시작하려면 먼저 최종 내레이션 기준이 될 음성 파일이 필요합니다. 이 저장소에서는 아래 두 가지 방식을 모두 허용합니다.

### 방법 1: 직접 녹음하거나 생성한 음성 파일 제공

가장 단순한 방식입니다. 사용자가 휴대폰, 녹음 프로그램, 다른 TTS 도구 등으로 만든 음성 파일을 작업 폴더 루트에 `voiceover.wav`로 넣어두면 됩니다.

권장 형식은 WAV입니다. MP3, M4A, FLAC, WebM 같은 일반 오디오 파일도 제공할 수 있지만, 영상 작업의 기준 파일은 최종적으로 작업 폴더 루트의 `voiceover.wav`로 맞춥니다. 필요하면 Codex가 FFmpeg로 변환한 뒤 전사와 싱크 작업을 진행합니다.
...
Voicebox 샘플 업로드는 WAV, MP3, FLAC, WebM 형식을 안내하고 있습니다. 이 저장소에서 사용할 최종 내레이션은 가능하면 WAV로 저장하고, 파일명은 `voiceover.wav`로 맞춥니다.
...
7. 해당 파일을 작업 폴더 루트에 `voiceover.wav`로 둡니다.

```text
YYYYMMDD_작업제목/voiceover.wav
```

Voicebox로 만든 음성을 사용할 때도 영상 제작 기준 파일은 항상 작업 폴더의 `voiceover.wav`입니다. Codex는 이 파일을 기준으로 길이 확인, 로컬 전사, 장면 타임라인, 싱크 검증을 진행합니다.

음성 샘플은 본인 목소리나 사용 권한이 있는 목소리만 사용합니다. 음성 원본과 생성된 `voiceover.wav`는 개인 자산이므로 기본적으로 GitHub에 올리지 않습니다.
