...
새 영상 제작 요청을 받으면 디자인 기준 확인 단계에서 `https://github.com/VoltAgent/awesome-design-md`의 컬렉션 또는 `design-md/` 폴더를 확인해 사용 가능한 `DESIGN.md` 종류를 사용자에게 리스트업합니다. 사용자가 이 중 하나를 선택하면 해당 `DESIGN.md`를 이번 작업의 스타일 레퍼런스로 읽고, 선택한 이름과 출처 URL, 적용할 색상/타이포그래피/레이아웃 단서를 작업별 디자인 브리프에 기록합니다. 사용자가 선택하지 않거나 기본값으로 진행하라고 하면 외부 `DESIGN.md`를 적용하지 않고 루트 `DESIGN.md` 규칙을 그대로 사용합니다.
...
- 사용자가 초기 영상 생성 프롬프트를 작성할 때는 첨부파일에 음성 파일과 스크립트 파일을 함께 추가하도록 요청합니다. 첨부 파일명은 자유롭게 두어도 되며, 음성 파일이나 스크립트 파일이 빠져 있으면 제작을 시작하기 전에 먼저 추가 요청합니다.
...
병렬 작업은 공통 타임라인이 확정된 뒤 구현 단계에서 가장 안전합니다. `audio/`의 기준 음성 파일, `transcript/sentences.json`, 승인된 장면 타임라인은 모든 구현 Agent가 공유하는 기준으로 잠그고, Agent끼리는 같은 파일이나 같은 도구 소스 폴더를 동시에 수정하지 않습니다.
